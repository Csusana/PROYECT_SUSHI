...
def login():
    if request.method == "POST":
        ...
        # Buscar en la lista usuarios_registrados si existe un usuario con email y password
...
def reservaciones():
    return render_template("services.html")  # o la que quieras
...
@app.route('/contact')
def contact():
    return render_template('contact.html')  # O lo que corresponda
...
def checkout():
    if request.method == 'POST':
        fname = request.form.get('c_fname')
        lname = request.form.get('c_lname')
        address = request.form.get('c_address')
        email = request.form.get('c_email_address')
        phone = request.form.get('c_phone')
        notes = request.form.get('c_order_notes')

        
        if not all([fname, lname, address, email, phone]):
